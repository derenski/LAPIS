...
synth_cont <- function(Y, W, return_weighted_average=TRUE){
  ...
  nonZeroDurations <- sort(unique(rowSums(W)[rowSums(W) > 0]), decreasing=T)
                                                     
  effectList <- foreach(duration=nonZeroDurations,
                                       .packages=c('quadprog'),
                                       .combine = 'c') %dopar% {
      
      control_times <- 1:(dim(Y)[2]-duration)
      
      T_0 <- (dim(Y)[2]-duration)
      
      treatment_times <- (dim(Y)[2]-duration+1):dim(Y)[2]
      
      train_X_omega <- t(Y[control_units,  control_times])
      
      test_X_omega <- t(Y[control_units,  treatment_times])
      
      treatedY <- Y[which(rowSums(W)==duration), , drop=FALSE]
      
      N_1 <- dim(treatedY)[1]

      train_y_omega <- colMeans(treatedY[ ,  control_times, drop=FALSE])
     ...
     # coeff_vector[coeff_vector < 1e-10] <- 0
      ...
      Y_treatment_only <- treatedY[ , treatment_times, drop=FALSE]
    ...
    bigNAMat <- array(NA, dim=c(length(effectList), max(lengths(effectList))))     
                                     
    for (index in 1:length(effectList)){
    
        bigNAMat[index, 1:lengths(effectList)[index] ] <- effectList[[index]]
    
    }
    
       numberOfEachUnitType <- table(rowSums(W))

   numPosUnitsEachType <- array(numberOfEachUnitType[names(numberOfEachUnitType) >  0],
                             dim=c(sum(names(numberOfEachUnitType) >  0), 1))
 
   unitsAvailableForEachObservedTime <- cumsum(rev(numberOfEachUnitType))

   minTime <- min(as.numeric(names(numberOfEachUnitType)))
maxTime <- max(as.numeric(names(numberOfEachUnitType)))


timeKeeper <- seq(minTime, maxTime, 1)

unitsAvailableForEachTime <- rep(NA, length(timeKeeper))
names(unitsAvailableForEachTime) <- timeKeeper

unitsAvailableForEachTime[names(unitsAvailableForEachObservedTime)] <- unitsAvailableForEachObservedTime

for (val in names(unitsAvailableForEachTime)){
    
    if (!is.na(unitsAvailableForEachTime[val])){
        
        next
        
    }else{
        
        whichIndexCorrespond <- which(names(unitsAvailableForEachTime)==val)
        
        allDataAfterVal <- unitsAvailableForEachTime[whichIndexCorrespond:length(unitsAvailableForEachTime)]
        
        lastNonNullIndex <- min(which(!is.na(allDataAfterVal)))
        
        unitsAvailableForEachTime[val] <- allDataAfterVal[ lastNonNullIndex ]
...
unitsAvailableForEachTime <- array(unitsAvailableForEachTime[2:length(unitsAvailableForEachTime)], dim=(length(unitsAvailableForEachTime)-1))

   weightedAverageMat <- (numPosUnitsEachType %*% (1/t(unitsAvailableForEachTime )))
            
   if (all(dim(weightedAverageMat)==1)){
       
      weightedAverageMat <- as.numeric(weightedAverageMat) 
    ...
    if (return_weighted_average){
       ...
       return(colSums(weightedAverageMat*bigNAMat, na.rm=T))
        
        }else{
        
        return(rev(effectList))
...
SDID_general <- function(Y, W,
                 iterations_for_coord_desc=100, return_weighted_average=TRUE){ 
  ...
  ### Estimate average effect for each duration group if staggered adoption design.
  effectList <- foreach(duration=nonZeroDurations,
                                       .packages=c('quadprog'),
                                       .combine = 'c') %dopar% {
    ...
    treatment_times <- (T_0+1):dim(Y)[2]
      
    T_1 <- length(treatment_times)
  
    WSubset <- W[c(control_units, which(rowSums(W)==duration)), ]
  
    YSubset <- Y[c(control_units, which(rowSums(W)==duration)), ]  
  ### Estimation of Omegas
  ...
  train_y_lambda <- rowMeans(Y[control_units,  treatment_times, drop=FALSE])
  ...
  matrix_of_ones <- array(1, dim=dim(YSubset))
  
  A_k <- array(0, dim=dim(YSubset))
  
  B_k <- array(0, dim=dim(YSubset))
  
  tau_k <- array(0, dim=c(dim(Y)[2], 1))
  ...
  for (k in 1:iterations_for_coord_desc){
      ...
      mu_k <- as.numeric(((t(omega_vector) %*% YSubset %*% lambda_vector) 
               - (t(omega_vector) %*% (A_k+B_k) %*% lambda_vector)
               -(t(omega_vector) %*% WSubset %*% (lambda_vector*tau_k)))
               /(t(omega_vector) %*% matrix_of_ones %*% lambda_vector))

      alpha_k <- (((YSubset %*% lambda_vector) 
                  - (B_k %*% lambda_vector)
                  -mu_k*(matrix_of_ones %*% lambda_vector)
                  -WSubset %*% (lambda_vector*tau_k))
                 /sum(lambda_vector))


      A_k <- matrix(rep(alpha_k, length.out=prod(dim(YSubset))), byrow=F,
                    nrow=dim(YSubset)[1], ncol=dim(YSubset)[2])


      beta_k <- (((t(omega_vector) %*% YSubset) 
                 - (t(omega_vector) %*% A_k)
                 -mu_k*(t(omega_vector) %*% matrix_of_ones)
                 -(t(omega_vector)%*% WSubset) * t(tau_k))
                /sum(omega_vector))


      B_k <- matrix(rep(beta_k, length.out=prod(dim(YSubset))), byrow=T,
                    nrow=dim(YSubset)[1], ncol=dim(YSubset)[2])

      tau_k <- as.numeric(((t(omega_vector) %*% (YSubset*WSubset)) 
                           - (t(omega_vector) %*% ((A_k+B_k)*WSubset))
                           -mu_k*(t(omega_vector) %*% WSubset))
                          /(t(omega_vector) %*% WSubset))

      tau_k[is.nan(tau_k)] <- 0
    
  }
      
      list(tau_k[treatment_times])
...
unitsAvailableForEachTime <- array(unitsAvailableForEachTime[2:length(unitsAvailableForEachTime)], dim=(length(unitsAvailableForEachTime)-1))
   ...
   if (all(dim(weightedAverageMat)==1)){
    ...
    # colSums(weightedAverageMat*bigNAMat, na.rm=T)
    ...
    # return(colMeans(bigNAMat, na.rm=T))
